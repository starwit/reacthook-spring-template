...
if ! yq write -i ${r"${helm_chart_dir}"}/values.yaml image.tag $RELEASE_VERSION ; then
...
## Replace RELEASE_VERSION with NEXT_VERSION
log "Setting Helm chart version to ${r"${NEXT_VERSION}"}-dev"
if ! yq write -i ${r"${helm_chart_dir}"}/Chart.yaml version ${r"${NEXT_VERSION}"}-dev ; then
  log "Helm chart version change failed!"
  exit 1
fi

git commit -a -m "Prepare next dev cycle: Setting version to ${r"${NEXT_VERSION}"}.dev"
if [ $? -ne 0 ]; then
  log "Commit failed. Exiting..."
  exit 1
fi
